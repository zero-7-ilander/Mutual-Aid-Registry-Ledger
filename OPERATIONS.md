...
- **Members own amendments on terms; operations and direction are the operator's.** Term changes (entry prices, dues, vesting, claim caps, trigger, cooldown) and any other Charter text go through the Member Amendment Proposal Process (below) and take effect only on ratification. Operational and directional decisions — process, tooling, document wording outside the Charter — are updated directly by the operator with partner approval, no ballot.
- **Charter changes run through the Member Amendment Proposal Process** (partner directive 2026-08-20; full spec in `GOVERNANCE.md`). Pipeline: Suggestion → Member Proposal → Qualification → Biweekly Tally → Amendment Draft → Member Ratification → Charter. Only the formal process can change the Charter. An active member publishes a public proposal, pays the 275t non-refundable Proposal Processing Fee (reason `REGISTRY-PROPOSAL`, statement-verified), and gathers 10 verified member supporters commenting publicly on the proposal post; proposals are qualified on a weekly review, prioritized in a biweekly tally (top 2–3 drafted; deterministic tie-break), drafted into amendment candidates, and ratified by a public vote (minimum 10 valid votes, For exceeding Against). The operator acknowledges suggestions and directs members through the process but never converts suggestions into proposals, polls, or amendments. Suggestions log: `ops/suggestions_log.json`; proposal tracker: `ops/proposals_log.json`; canonical suggestion reply: `ops/dm_templates.json` `governance_suggestion`.
