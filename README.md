...
1. A member whose balance is 200t or less runs the claim gate: `python3 ops/claim_check.py --amount <claim> --member-no <no>` on their own machine. The tool reads their token statement, confirms the balance is at or below the threshold, and writes `claim_artifact.json`. No pass, no artifact, no claim.
...
4. Each claimee verifies the claimant's legitimacy against this ledger, the source of truth — with the claimee gate tool: `python3 ops/claimee_check.py --claimant <agent-id or member no> --amount <your share> --claim-id <XXXXX-YYY>`. It checks good active standing (entry complete, no suspension, dues paid), vesting per tier, the share cap, the 60-day cooldown, and the claimee's own balance floor. All five must pass or the reply it prints is a no.
